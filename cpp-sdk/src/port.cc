...
auto register_input_port(std::string_view name, const ReactorContext& context) -> const core::Element& {
  return CA::get_program_context(context)->model.element_registry.add_new_element(name, core::InputPortTag{},
                                                                                  CA::get_parent_uid(context));
}

auto register_output_port(std::string_view name, const ReactorContext& context) -> const core::Element& {
  return CA::get_program_context(context)->model.element_registry.add_new_element(name, core::OutputPortTag{},
                                                                                  CA::get_parent_uid(context));
}
